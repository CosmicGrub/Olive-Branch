...
> **Status, v0.49.69 — the gate now also runs on the actual send path, not
> only the read-only display above.** Until this pass, `gate()` had exactly
> one real caller: `GET /now`, which only ever REPORTS whether the child is
> reachable — it never stopped anything from being sent. The one real
> production caller of `notifyDevices()` (`packages/transport/src/
> notify.ts`, the calls route's `call_incoming` push) rang a sleeping or
> in-school child's device with no check at all. Now `notifyDevices()`
> itself resolves `childCtxFor()` + `gate()` once, before any device is
> touched, for every CHILD-targeted send — a blocked batch is skipped
> entirely, not fired and left for the OS to (maybe) silence. Scoped
> deliberately narrow: this is a hard skip, not the deferred-retry
> `deferTo` otherwise supports — no queue or scheduler in this codebase can
> re-attempt a push later yet (`tools/scheduler.mjs`'s own header already
> names that as a separate, open gap), and a live call has nothing
> meaningful to defer TO regardless. `NotifyInput.priority` defaults to
> `'normal'`, so `call_incoming` is now itself subject to the same
> asleep/school gate as any other arrival — nothing in this codebase has a
> concept of an emergency call that should bypass it, and this pass does
> not invent one. The gate remains CHILD-scoped only; a guardian-targeted
> send (`{userId}`) never reaches it — a parallel "guardian quiet hours"
> concept is a genuinely separate, unscoped product question, not decided
> here. See CHANGELOG v0.49.69.
...
**ChildHome's own tile grid — real 3-tier hierarchy, v0.49.65 (intuitivism pass, sub-project 2).** The child shell's home screen (`child_home.dart`, MARKUP `childHome`) renders 9 real destinations — Homework, Play together, More games, My list, Messages, My day, Storyteller, Show & tell, More for you — previously one flat, equal-weight grid with a HARDCODED `crossAxisCount: 2` (`form_factors.dart`'s real posture system, already used by `game_picker.dart` since v0.49.17, was imported nowhere in this file — a real, independently-found inconsistency, now closed). Real hierarchy replaces it: **Hero** (My day — the one tile this document calls a "signature element," §8.2.2 below) full-width and largest; **Featured** (Play together, Messages, Storyteller, Show & tell) larger, posture-aware cells; **Standard** (Homework, More games, My list, More for you) at the previous tile size. Every placement traces to a real citation where one exists (Storyteller's own "a story every night... draws 6,570 of them," §9.11; Show & tell's own self-critique that it has been under-treated as secondary, §9.10) or is disclosed as a design judgment call where it doesn't — see `docs/superpowers/specs/2026-08-31-intuitivism-navigation-density-design.md` for the full, cited account. Hierarchy is communicated by size/width/type-scale alone — never her own §8.6.2 accent colour, never usage-analytics-derived (P2). The ad-hoc local-play games' own eventual placement (Featured tier, once PR #85/#87's independent navigation-file changes are reconciled) is answered in that spec but deliberately not wired here.

**GuardianHome's own tile grid — the SAME real 3-tier hierarchy, v0.49.71 (intuitivism pass, sub-project 3b).** `_Tile` was extracted into a new shared `tiered_tile.dart` (`TieredTile`, identical behavior — ChildHome's own migration onto it is a pure rename, proven by its full existing test suite passing unmodified) once GuardianHome needed the same shape rather than a second, drifting `_GTile` copy. GuardianHome's 11 real destinations (`guardian_home.dart`, MARKUP `guardianHome`) — previously one flat, equal-weight grid — now render as: **Hero** (Message banking, full-width, outside the grid); **Featured** (Availability, Send-time guard, Meds & care, Emergency card — `secondaryContainer`); **Standard** (Handover notes, Exchange, Expenses, Morning briefing, Care note, More — `primaryContainer`, unchanged). Every placement traces to the user's own stated answer about which tiles feel most urgent/frequent as a real guardian, not usage data (P2 not triggered) — see `docs/superpowers/specs/2026-09-12-intuitivism-guardianhome-tiering-design.md`. The ribbon above (Ivy's day bars, Call Ivy) is untouched, real live data, not a tile. A real, pre-existing-class overflow bug was found and fixed along the way: Featured's larger icon/type-scale, combined with this screen's OWN shared (not per-tier) `mainAxisExtent`, reintroduced the exact 344px-width wrap-overflow class §8.11.1 already documents for this file, now also present in a real ~372-380px phone-width band the old 165px breakpoint routed into too short an extent — both breakpoint values grew (136/170, was 128/165) to close it, confirmed by real widget tests pinned to exact widths.
...
**One screen for every choice she has — shipped v0.49.66.** A real,
separate consolidation from the ad-hoc-play navigation question directly
above (that one is still genuinely undecided; this one is a different,
already-existing split this pass closes). Before this pass,
`child_home.dart`'s "Play together" tile opened `GamePickerScreen`
(the age-gated `GameKind` catalogue grid) and `games_hub.dart`'s separate
"More games" tile opened a second screen for the checkers/chess/
battleship/word-game cluster — a real, avoidable split with no product
reason behind it. `GamePickerScreen` gains a new `extraSections` slot
(a `List<Widget>?`, null by default — every existing call site and test
renders exactly as before this field existed), rendered below the grid
inside the SAME scroll view; `games_hub.dart`'s own section list is
extracted into a new `MoreGamesSections` widget (pure extraction, same
widget tree, now reachable by a second caller) that `child_home.dart` and
`guardian_more.dart` both now pass straight through. `game_navigation.dart`
(new) extracts the `GameKind → screen` switch itself, shared by both real
call sites, so they can never independently drift the way two hand-
maintained copies of the same switch eventually would — a real, present
risk this consolidation itself would otherwise have doubled, since the
switch now needs to stay correct in one more place that reaches it.
`GamesHubScreen` itself is untouched and still exists in its own right for
any caller that wants just that half on its own. Proven, not just
asserted: a new test drives the real path end to end — tap "Play
together," scroll the SAME resulting screen, reach a real `GameCheckers`
by tapping "Checkers" — confirming the consolidation actually works, not
only that the grid still renders. See CHANGELOG v0.49.66.

**The jokebook — shipped v0.49.67.** A fixed, hand-written library of
kid-friendly jokes (dad jokes, puns, wordplay, plain silliness, knock-knocks)
she reaches for when she wants a quick one: reads the setup, taps for the
punchline — the tap IS the comic timing — asks for another, stars the ones she
likes. Nothing is generated; every joke was read by a person before it
shipped, which is why it needs no safety sweep. Age-gated by `forAge()`
exactly as the games catalogue is, but the floor is a comprehension floor, not
a content rating: every joke is appropriate for every age; the floor only
stops a joke she can't get yet from landing flat. Nothing is gated above 12.
Reached as one more `extraSections` block ("Just for laughs") behind the same
"Play together" door — not a new ChildHome tile, for the reason the paragraph
above gives. "Tell Dad this one" sends nothing and never claims to: there is
no child→guardian send in this client yet, so it hands her the joke large
enough to read out on a call or hold up to the camera, and says so. P2
governs the favourites: a favourite is an id and a timestamp — no count, no
streak. See CHANGELOG v0.49.67.

**GamePickerScreen's Recommended row — shipped v0.49.70.** Intuitivism pass,
sub-project 3a (`docs/superpowers/specs/2026-09-12-intuitivism-gamepicker-
recommended-design.md`). A guardian-curated favourites mechanism, mirroring
the jokebook's own `star`/`unstar`/`favouritesFor` shape exactly
(`packages/games/src/favorites.ts`, `game_favorites_logic.dart`), plus a
complementary age-unlock signal (`newlyUnlocked()` — a game whose `minAge`
she has crossed since her last visit to this screen, derived purely from her
real age and `child.birth_date`, no play history or usage tracking of any
kind) and a "surprise me" random pick (`randomGame()`, the identical
`randomJoke(age, excludeId, pick)` contract). Favourites-considered-first,
age-unlock fills any remaining slots, each game appearing at most once even
if it qualifies both ways; the row is entirely absent — not an empty state —
whenever there is nothing to recommend, the same "honest absence" posture
`careNotesFor()`'s own no-notes handling already established. New
`guardian_game_favorite` table (`db/migrations/0030_game_favorites.sql`),
`..._no_child` RLS exactly like every other guardian-only preference table
in this schema (`medication`/`care_note`), keyed on the GUARDIAN, not the
child — resolved per child the same `guardiansOfChild()` join
`guardian_availability_window` already uses. A second new table,
`child_game_picker_state`, holds the one nullable `age_at_last_open`
integer the age-unlock signal needs — child-owned RLS, the inverse of
`guardian_game_favorite`, written by the CHILD's own session on open (her
age, her visit, no guardian involvement), never a log of opens. New
`GET`/`PUT /v1/children/:childId/game-favorites`, reusing `action:
'settings'` verbatim from the theme-preference route (sub-project 1) —
guardian writes her favourites (full-replace, the same upsert-the-whole-
preference shape `setChildTheme()` already uses); the child's own PUT
(no body at all) records her visit, her real age computed server-side from
`birth_date`, never trusted from the client. `GamePickerScreen` gains four
new optional constructor params (`favoriteKinds`/`ageAtLastOpen`/
`onToggleFavorite`/`onSurpriseMe`) — still a plain, non-fetching
`StatelessWidget`; the live session lives in a new `LiveGamePickerScreen`
wrapper instead, the same split `court_export.dart`'s own Live/static pair
already established. The star toggle on `_GameCard` renders only when a
GUARDIAN opened the screen (`guardian_more.dart`'s "Play together" tile —
this spec's only guardian-driven favoriting surface); a child-opened screen
(`child_home.dart`) gets her real Recommended row and Surprise-me button but
never the star — she sees the result of a favorite, never the mechanism. P2
throughout: no favorite-count, no unlock-count, no order-of-favouriting, no
streak, ever surfaced to her. See CHANGELOG v0.49.70.
...
> **Guardian kiosk PIN made a REQUIRED step, v0.49.73.** The "Auth" row
> above has read "device-bound token + PIN (children)" since this table was
> first written, but said nothing about the GUARDIAN's own kiosk PIN
> (`pin_credential`, real, scrypt-hashed, wired via `POST /v1/me/pin` since
> v0.49.30) — which was, until now, optional. Onboarding & Guardian Access
> sub-project 1 (docs/superpowers/specs/2026-09-12-onboarding-identity-pin
> -design.md) wraps `guardian_setup.dart`'s existing, already-real PIN
> section in a small linear stepper: a new "Finish setup" action, disabled
> until `setGuardianPin` has succeeded at least once THIS session, or
> already enabled on a return visit where a PIN already exists (a new
> `hasPin` field on `GET /v1/me`, backed by the existing `pinCredentialFor()`
> — no new table, no new query shape). The still-stubbed passkey section
> above is completely untouched — a separate, honestly-labeled, non-blocking
> section, not part of the required path. This matters beyond this one
> screen: sub-project 2's entire premise (PIN-gated parental controls) needs
> a PIN to reliably exist, and an optional one today would have left that
> feature unreachable for any family that skipped it. See CHANGELOG v0.49.73
> for the full account.

> **Device pairing & provisioning, v0.49.74.** Every live build until now was
> provisioned entirely at compile time (`main_live.dart`/`main_live_guardian
> .dart`'s own `--dart-define=OLIVE_CHILD_ID=...`/`OLIVE_GUARDIAN_ID=...`),
> with nothing ever persisted on-device — this app had zero `SharedPreferences`
> (or any storage) usage anywhere. Getting a new physical device working meant
> a rebuild with the right flags; there was no path for a guardian to just add
> a device from within the app. docs/superpowers/specs/2026-09-12-device
> -pairing-provisioning-design.md closes that gap for the common real cases (a
> lost/broken/new kiosk tablet, a guardian's second phone) — WITHOUT touching
> how families or children get created in the first place (still
> `seed-dev.mjs` only; see that spec's own "Explicitly out of scope").
>
> Two new tables (`db/migrations/0032_device_pairing.sql`):
> `device_pairing_code` (a short-lived, single-use QR id + 6-digit fallback)
> and `paired_device` (what a redeemed code actually mints). Real RLS on
> both — owner-write for a code's creating guardian, family-scoped (reusing
> kiosk-pin/verify's own "every live guardian of this child" scoping) for
> `paired_device`, and a `system`-role redeem path mirroring
> `guardian_invite`'s (0014) own accept-flow precedent EXACTLY, since the
> redeeming device has no session at all yet.
>
> **A real session/auth change, the other genuinely new mechanism**: signed
> sessions (`issueSession`/`readSession`, `packages/auth/src/auth.ts`) can now
> carry a `deviceId` claim, minted only by `POST /v1/device-pairing/redeem`.
> `packages/api/src/api.ts`'s `handle()` checks it on every request: a
...
> runs. Sessions from `DEV_LOGIN` (and every other existing session-issuing
> path) carry no `deviceId` and are completely unaffected — a deliberate,
> disclosed non-goal, not a gap.
>
> **Five real routes** (`server/routes.mjs`): `POST .../device-pairing-codes`
> (guardian, live edge to the child, re-verifies the caller's own PIN via the
> existing `attemptPinFor` machinery), `POST /v1/me/device-pairing-codes`
> (any guardian, bound to their own identity), `POST /v1/device-pairing/redeem`
> (no session; validates expiry/revocation/redemption/role-match, a numeric
> -code lockout mirroring the kiosk-PIN's own `PIN_MAX_ATTEMPTS`/
> `PIN_LOCKOUT_MS` shape, atomic mint-session+insert+mark-redeemed),
> `GET .../paired-devices` (guardian, family-scoped list), and
> `POST .../paired-devices/:deviceId/revoke` (guardian, live edge). A sixth,
> `POST /v1/device-pairing-codes/:codeId/cancel`, was added beyond the
> design spec's own Routes list — a disclosed judgment call, since the
> spec's own Flow section requires a working Cancel-before-use button and the
> migration's own `revoked_at` column exists for exactly that; see the PR
> description for the full account.
>
> **Client**: a new "Add a device" area off Guardian More (parallel to the
> existing guardian-invite entry point) — role picker, PIN re-entry, then a
> real QR (`pretty_qr_code` — `qr_flutter`, the prior default, has zero
> listed maintainers and no release in roughly three years, confirmed before
> switching) plus the 6-digit fallback and a live countdown. A new pairing/
> redeem screen (scan via `mobile_scanner`, or type) shown at boot whenever
> there's no stored identity AND no dart-define set — `bool.hasEnvironment`,
> not a defaultValue comparison, is what tells the two apart. On success,
> `{sessionToken, deviceId, role, targetId}` persists via
> `flutter_secure_storage` — the FIRST on-device persistence this app has
> ever needed. A new "Paired devices" list (guardian-only) with per-row
> revoke. `device_revoked` is handled as its own distinct exception
> (`DeviceRevokedException`, api_client.dart) — never folded into the
> generic network-failure path.
>
> **Verified so far**: 55 fake-pool route-contract assertions
> (`server/test/device_pairing_route_test.mjs`) and 46 real-Postgres/real-RLS
> assertions (`packages/db/test/device_pairing.test.mjs`, including the real
> HTTP route end to end: redeem, then a genuine revoke, then the SAME token
> refused as `device_revoked` on a completely different route) — both
> against a real Postgres, not a fake's opinion of one. 41 new/extended Dart
> widget assertions across five files, `flutter analyze` clean. **What this
> has NOT verified**: any of it on a real physical device — the same honest
> `UNVERIFIED` posture every Dart file in this client already carries until
> a real-hardware pass confirms otherwise.

> **Automatic First-Run Detection, v0.49.76.** The "Guardian kiosk PIN made
> a REQUIRED step" entry above (v0.49.73) made the PIN mandatory as
> *policy*; nothing enforced it until a guardian happened to open Guardian
> Setup herself. This closes that gap for real: `main_live_guardian.dart`
> fetches `GET /v1/me` after identity resolves (pairing or dart-define,
> identically) and, when the new `hasPin` field reads `false`, renders
> `GuardianSetupScreen` in place of `GuardianHome` outright — a hard,
> non-dismissible gate, re-checked on every launch. `GuardianSetupScreen`
> itself required NO code change; only what decides the initial screen did.
> `setGuardianPin`/`checkExistingPin` are wired the identical way
> `guardian_more.dart`'s own existing hub entry point already wires them.
> Completing setup re-invokes the SAME boot function that got her here
> (`_bootLiveApp`), which re-resolves `hasPin` fresh and this time finds it
> true — "the very next boot proceeds to GuardianHome normally"
> (§8.5.5's own line), implemented literally as a fresh boot rather than a
> local state flip. The CHILD-side half of this same pass — `hasOnboarded`,
> the pre-`KioskShell` onboarding boot branch — is §8.5.5's own account
> above, not repeated here since it touches no guardian-auth mechanism at
> all. See CHANGELOG v0.49.76 for the full account, including this pass's
> own disclosed judgment calls.
...
| ~~`health_check`'s own tracked-table list could silently fall behind a real RLS migration~~ | **CLOSED v0.49.69.** A fourth real recurrence of the exact failure class `tools/health-alert.mjs`'s own header already named twice (0006, then 0028): `db/migrations/0026_medications_emergency_card.sql` gave `medication_dose` real, forced RLS, but only its siblings `medication`/`medical_record` were ever added to `health_check`'s tracking list (migration 0028's own fix for the prior instance of this same gap). Found by a new automated backstop, not by manual inspection — `packages/db/test/rls_coverage.test.mjs` derives the REAL RLS-enabled table list from `pg_class` itself and asserts none is missing from `health_check`'s own view SQL (parsed from its live definition) and that every RLS-enabled table also has `FORCE ROW LEVEL SECURITY` — on its very first real run, before this pass added `medication_dose` to migration `0029_rls_coverage_fix.sql`'s `CREATE OR REPLACE VIEW health_check`, it failed exactly where the gap was. `health-alert.mjs`'s own header no longer names a specific migration number at all (it had already gone stale twice on that exact line) — it now points at this test as the real, standing defense instead. 5 new assertions, registered in `tools/verify.sh`. |
| ~~A message's retention clock never actually shortened when she opened it~~ | **CLOSED v0.49.69.** `packages/messaging/src/pipeline.ts`'s `retentionOnOpen()` (shortens-never-lengthens, real and tested since before this pass) had no production caller — `POST /v1/children/:childId/inbox/:messageId/opened` marked a message opened and stopped there, so an unpreserved artifact's expiry never moved up on open, only ever the flat default clock from capture. A real, live bug was found wiring this in, not merely a missing call: the naive first attempt ran the `media_artifact` UPDATE under the route's own `child`-role session — `media_artifact`'s RLS (0023) admits only guardian-tier roles and `system`, never `child`, so under FORCE ROW LEVEL SECURITY the UPDATE silently affected zero rows, no error, nothing to notice. Fixed with a new `applyRetentionOnOpen()` (`packages/db/src/pool.ts`), scoped through `withSystemSession()` exactly like `mediaArtifactFor()` already is, called from the route after the open itself succeeds. 8 new assertions (`server/test/inbox_route.test.mjs`), covering real shortening, the never-lengthens guarantee, idempotent re-opens, and a preserved artifact's clock staying untouched. |
| ~~Guardian availability notes had no tone guard, and no way to author one at all~~ | **CLOSED v0.49.69.** `server/routes.mjs`'s `invalidAvailabilityBody()` checked only that a `note` was a string, nothing about content — and `availability_screen.dart`'s `_dayRow()` had no field to type one into; a note only ever arrived pre-existing in fetched data and was displayed read-only. An availability note is served straight to the CHILD's own session (`guardian_availability_child_read`, migration 0010) — more exposed than a care note, which `writeCareNote()` always marks `visibleToChild: false`. Closed by a new `AVAILABILITY_NOTE_BANNED` export (`packages/guardian/src/guardian.ts`, reusing `CARE_NOTE_BANNED`'s own list directly — one word list, not two to keep in sync by hand), enforced in `invalidAvailabilityBody()` before persistence, and a real, bounded `TextField` in `_dayRow()` (60-character UI cap; the tone guard itself is the real, server-side enforcement). 2 new server assertions, 3 new Dart widget tests. |
...
### 8.5.4 She is asked her gender, once, and it changes nothing (v0.49.73)

**Onboarding & Guardian Access sub-project 1** adds a fourth step —
`onboarding_gender.dart`, reached after age, before who — with the same
posture as every step above it: **Boy**, **Girl**, or an explicit **Skip**,
never a forced choice. Real as of this pass in one specific sense none of
the first three steps has ever been: a real tap genuinely persists, via a
new `PUT /v1/children/:childId/profile` route and a new, child-owned,
guardian-excluded `child_profile` table (`db/migrations/0031_child_profile
.sql`) — the FIRST route in this whole pipeline that any first-run screen's
tapped answer has ever actually reached. `onboarding_logic.dart`'s (Dart) /
`onboarding.ts`'s (TS engine) own `outcome()` has computed a real result
object since before this pass; nothing ever ingested it. A Skip writes
nothing at all — no row, not even a null one, matching this app's own
honest-absence convention.

**This is not §8.5.0's rejected idea, revisited.** §8.5.0 documents a
near-identical shape — one modal reading a self-reported signal to decide
device authority — and rejects it outright: routing real guardian authority
off a child's tapped, unverified answer is the same failure `AgeStep`
already guards against one layer down, at much higher stakes. Gender here
drives **nothing** — no route, no screen, and no future feature this pass
builds ever reads it back (there is deliberately no guardian-facing read
route at all yet); it is captured honestly and left unused, the identical
posture `guardian_setup.dart` already takes with its own still-stubbed
passkey section ("no such service exists yet in this preview build," said
outright). **P2 is not triggered**: a bare, unscored, non-comparative fact
about her, the same shape a favourite kind or a starred game already takes
— nothing here is a count, a rank, or a streak, and nothing here is ever
shown back to her as a status. Full design spec: `docs/superpowers/specs/
2026-09-12-onboarding-identity-pin-design.md`.

### 8.5.5 First run finds her automatically now (v0.49.76)

Every version above this line still describes a screen she could *reach* —
name, age, who, gender. None of them describe *arriving there
automatically*. Until this pass, a child who finished onboarding and a
child who had never opened the app were server-side indistinguishable
(§8.5.4's own Skip-writes-nothing behaviour was exactly why), and a
guardian with no PIN could use the app indefinitely — nothing ever checked
`hasPin` until she happened to open Guardian Setup herself. Both gaps were
real, disclosed, and repeatedly re-stated as deferred by three consecutive
shipped sub-projects (v0.49.73, v0.49.74, v0.49.75) without ever closing.

**The signal fix.** `onboarding_gender.dart`'s `_finish()` now calls
`PUT /v1/children/:childId/profile` unconditionally — on a real tap AND on
Skip. An explicit `null` gender is a real, meaningful answer ("onboarding
completed, gender declined"), written as a row with a real `set_at`, not
the fabricated state §8.5.4's own account warned against. `GET /v1/me`
gains `hasOnboarded`, symmetric with `hasPin`: `null` for a guardian,
`true`/`false` for a child, computed from `child_profile` row existence.

**Two hard, non-dismissible boot gates**, checked fresh on every launch,
identically for a paired device or a dart-define-provisioned one:

- Guardian side (`main_live_guardian.dart`): `hasPin === false` renders
  `GuardianSetupScreen` in place of `GuardianHome` outright. That screen's
  own code is unchanged (§11's own account below) — only what decides the
  initial screen changed.
- Child side (`main_live.dart`): `hasOnboarded === false` runs the
  *existing* onboarding sequence inside a new pre-`KioskShell` boot root,
  mirroring the device-pairing screen's own boot-branch shape rather than
  inventing a second pattern for "content that must run before lock-task
  engages." Kiosk lock-task cannot engage while she is mid-onboarding —
  the widget that engages it does not exist anywhere in that branch's own
  tree. Only a genuine finish (which now always writes `child_profile`,
  per the signal fix) proceeds into the real kiosk shell.

**Explicitly declined**: a "skip for now"/dismiss path for either gate —
both are hard gates by design, the same posture §8.5's own "nothing here
may fail and no step may trap her" rule does NOT extend to a REQUIRED
policy a prior sub-project already declared (guardian PIN, v0.49.73's own
§11 account below). Full design spec: `docs/superpowers/specs/2026-09-14
-automatic-first-run-detection-design.md`.
...
> **Status, v0.49.64 — §5.23.1's "which button she tapped" is a real choice
> now, not a description.** `call_knock_screen.dart`'s "Answer" and "Just
> talking" buttons existed before this pass but were honestly identical in
> behavior — both led to the same video join, since no ported logic existed
> yet to make them differ and inventing one would have been exactly the
> fabrication §0 forbids. `call_modes.dart` (new, a deliberately partial 1:1
> port of `packages/live/src/modes.ts` — §5.23.1's `CallMode`/
> `answerOptions()` ported; push-to-talk and bedtime mode explicitly NOT,
> since nothing calls them yet) makes the difference real: "Answer" joins
> with video, "Just talking" joins audio-only, carried through a new
> `CallScreen.initialMode` parameter. **Resuming asks first** (the paragraph
> above) is also now real, not just specified: an unexpected disconnect
> that interrupted a call with the camera on holds it off after a
> successful reconnect and waits for an explicit tap, rather than silently
> resuming transmission the moment the network recovers. Also real now:
> relay-only ICE as defense-in-depth (LiveKit's SFU has no peer-to-peer path
> to disable in the first place, but the client now also forces
> `RTCIceTransportPolicy.relay` directly), and `adaptiveStream`/`dynacast`
> (a `RoomOptions` this client never constructed at all before this pass).
> `camera_controls.dart` (new, a 1:1 port of `packages/live/src/camera.ts`,
> §5.24) ships with real tests but is deliberately NOT yet wired into
> `call_screen.dart` — shipping the logic ahead of its UI integration is
> this pass's disclosed scope, not an oversight. No dedicated MARKUP screen
> entry — `call_screen.dart`/`call_knock_screen.dart`, the tracked screens,
> are unchanged in their rendered shape (the same three answer buttons); only
> what happens after the tap differs. See CHANGELOG v0.49.64.
...
**v0.49.72 is the first pass to give `Posture.foldCover`/`Posture.
foldTabletop` real, posture-SPECIFIC treatment, rather than folding them
into the generic `columnsAt()` narrow/wide split every entry above uses.**
Two new shared widgets — `CoverCollapse` (`ChildHome`/`GuardianHome` show
only the ribbon/presence area and the Hero tile at foldCover, everything
else behind a "More" tile) and `TabletopSplit` (`storyteller_screen.dart`/
`showcase_screen.dart`/`homework_screen.dart`/`call_screen.dart` split
viewing content above the hinge from controls below it at foldTabletop) —
see §8.11.2's own status note immediately below for the fuller account and
CHANGELOG v0.49.72.
...
**Something does, as of v0.49.72.** `TabletopSplit` (new
`client/lib/tabletop_split.dart`) renders exactly this — `viewing` above the
hinge, `controls` below it, a real `Column` split (each half independently
scrollable) — at `Posture.foldTabletop`, and the caller's own unsplit
single column at every other posture. Wired into `call_screen.dart` (video
above, mute/hang-up/camera below — this section's own "the only hands-free
call posture" made literal) plus `storyteller_screen.dart`,
`showcase_screen.dart`, and `homework_screen.dart`. Real hinge-angle sensing
(`FoldingFeature`) is still not wired — the split is an even division of
the reported window, not a measurement of where the physical hinge sits;
`isTableTopPosture`, a lighter real signal, was considered and deliberately
left for a later pass. `Posture.foldCover` gets the matching treatment on
the child/guardian home screens — `CoverCollapse` (new
`client/lib/cover_collapse.dart`) shows the single most load-bearing
tile at that posture instead of the full layout squeezed narrow. See
CHANGELOG v0.49.72 and `docs/superpowers/specs/2026-09-12-intuitivism-
fold5-layout-design.md` for the full account.
...
### 21.11 Contrast — PIN-gated parental controls (visibility & pacing) is NOT a rung

**Status: BUILT, v0.49.75.** `docs/superpowers/specs/2026-09-13-parental-
controls-pacing-design.md` ("sub-project 2" of the onboarding/parental-
controls arc — sub-project 1, the required guardian PIN, shipped in v0.49.73
specifically to unblock this). `guardian_activity_override`
(`db/migrations/0033_guardian_activity_override.sql`) is the first
guardian-writable table that has ever actually controlled what a child can
access — hide/show any ChildHome tile, game, joke, or drawing/activity
outright (`visible`), or bidirectionally adjust/pre-empt the existing
per-item age-based unlock (`min_age_override`, `revealed_at`).

Placed here, beside §21, because a reader who has just read the ladder's own
discipline will reasonably ask whether this is a seventh rung. It
deliberately is not, on two axes this section's own §21.1 two rules govern:

1. **Bidirectional, not a ratchet.** §21.1's Rule 1 — "a rung is irreversible.
   A guardian may never lower one. `canGuardianRevoke()` returns `false` and
   there is no inverse function" — governs privacy/authority grants, a
   materially higher-stakes domain than game/content variety. A guardian can
   raise OR lower `min_age_override`, flip `visible` either way, or set/clear
   `revealed_at`, in any order, any number of times — no column on this
   table is append-only or one-directional, by direct confirmation during
   this feature's own scoping. Hiding "Show & tell" today and un-hiding it
   next week is a normal, expected use of this screen, not a workaround.

2. **Single-guardian, not dual-consent.** The ladder's OWN dual-guardian
   consent machinery (`maturation.ts`'s `consentingGuardians.length < 2`)
   was seriously considered as precedent here, and rejected — because it
   turns out to be unwired: that check is a bare array-length test the UI
   satisfies with a single guardian self-ticking a checkbox on the other's
   behalf, no server route, no DB table, no real second-party verification
   anywhere in this codebase. Building real dual-guardian consent would be a
   genuinely new, separate capability. This feature instead matches this
   app's actual, universal existing precedent (§17.1, "the product must be
   fully usable with exactly one guardian"): any one live guardian with a
   real edge (`actor_has_edge()`) can write any guardian-scoped setting —
   theme, PIN, invites, and now visibility/pacing — unilaterally, gated only
   by a fresh PIN confirmation at screen entry
   (`POST /v1/me/verify-controls-pin`), never per-write (re-requiring a PIN
   on every toggle flip would make the screen painful to actually use — the
   existing kiosk-PIN/device-pairing precedent is for one-shot, high-stakes
   actions, not a settings screen with many small edits per visit).

RLS mirrors `child_theme_preference` (0017) exactly: guardian FOR ALL via
`actor_has_edge()`, child SELECT-only via `current_child()`, ENABLE+FORCE,
no child write policy at all — with one deliberate divergence disclosed in
0033's own migration header: no system-role read policy exists on this
table, so `GET /v1/children/:childId/activity-overrides` (dual-purpose —
a child's own session filters her catalogue, a guardian's session renders
the settings screen) runs as the REAL calling principal, making RLS alone,
not an app-layer comment, the thing deciding scope.

Two things this pass explicitly does NOT do, matching the design spec's own
"Explicitly out of scope": it does not invent a `minAge` for a surface that
never had one (storyteller, homework, drawing/activities keep visibility-
only overrides), and it does not build the unified activity-catalogue
abstraction `activity_key`'s bare-string shape was deliberately chosen to
avoid needing — games/jokes/activities keep their own independent internal
catalogues; this table only ever stores an override on top of whatever each
one already computes.

**v0.49.78 — the deferred IA half, now built.** This section's own scoping
never claimed the resulting screen would be usable at scale — flat single-
level lists of 88 (Visibility) and 79 (Pacing) rows, explicitly named and
deferred at the time theme #4 shipped (its own spec's "Explicitly out of
scope" section names this exact theme). `docs/superpowers/specs/2026-09-15-
parental-controls-ia-rework-design.md` (UI/UX review theme #5, fifth of
seven) builds that deferred half: both tabs' per-category `ExpansionTile`s
gain a nested sub-group level — Jokes (59) by `joke_logic.dart`'s own
`JokeCategory` enum (5 sub-groups), Games (21) into "Games"/"More games" (2
sub-groups, matching the main game picker's own real framing of these items,
deliberately coarser than `games_hub.dart`'s own 4-way section split) — plus
a per-tab search field reusing `story_library.dart`'s `_SearchShelf` pattern
directly. Three targeted fixes ride along: joke-title truncation (`maxLines`
1→2), a genuinely-confirmed Pacing-tab overflow at this app's 344px Fold5-
cover floor + 2.0x text scale (verified BEFORE fixing, per the spec — a real
`RenderFlex overflowed` failure, not assumed), and the Reveal-now button's
visual separation from the age-stepper, the latter two resolved together by
moving the trailing controls out of `ListTile.trailing` onto their own line.
The underlying `guardian_activity_override` table, its RLS, and every route
above are entirely untouched — presentation-layer only, exactly as scoped.
...
**v0.49.77 — the deferred spacing/radius half, now built.** This section's
own opening line scoped itself to "the design-token foundation only,"
explicitly deferring spacing/radius/elevation tokens as later work that
never happened until now. `docs/superpowers/specs/2026-09-15-design-tokens-
named-migration-design.md` (UI/UX review theme #4) builds that deferred
half: a new `client/lib/design_tokens.dart` (`AppSpacing`/`AppRadius`,
values matched to this codebase's own already-dominant `EdgeInsets`/
`BorderRadius` literals, not an invented scale), applied narrowly to the
review's own four concretely-named problem spots (ChildHome's grid-collapse
bug, the 5-game foldTabletop padding duplication, the coordination-card
family's inconsistent margin/padding, and story_library.dart/
storyteller_screen.dart's two genuine navigation rows onto `HubTile`) —
deliberately not a retrofit of the ~100 other files with their own one-off
spacing drift, the same "refine, don't redesign" budget this section's own
color pass already worked within.

---

*End of MASTERFILE v0.49.73. Amend in place. Bump version. Log in CHANGELOG.
