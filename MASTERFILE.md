...
**Still open, same shape the four closed pieces were in before their own closures:** care notes (`care_note.dart`) and letters (`letters_screen.dart`) — each has a real, tested screen and real domain logic, and neither has a database table, a server route, or a single production write path behind it. Not built here — disclosed, tracked, and deliberately not silently expanded into this pass's own scope. (`packages/school` — the §11.5 school layer — is a separate case, not part of this row: it is deliberately non-integrating by design, "Olive does not integrate with school systems," per its own header, not an unbuilt backend.) |
...
*End of MASTERFILE v0.49.55. Amend in place. Bump version. Log in CHANGELOG.
