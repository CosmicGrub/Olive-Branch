...
| **`GuardianHome` has no live-data screen** | **REAL, PROVEN GAP — confirmed v0.49.15, still true at v0.49.39. Not a dead wire; there is no wire.** `guardian_home.dart`'s dual-clock/Day Ribbon widget (§8.2.1–§8.2.3) takes every field — child's local time, zone, ribbon bands, overlap label — as a plain constructor argument; the file has no fetch of any kind. No `guardian_home_live.dart` exists anywhere in this client, unlike `child_home_live.dart`'s own equivalent for the child side. `main_live_guardian.dart` (v0.49.30, the live guardian entry point) deliberately boots straight into `GuardianMoreScreen` instead of the real `GuardianHome` — its own header states why: the ribbon data `GuardianHome` needs has no server route behind it anywhere in this codebase, and `api_client.dart` only ever declared a path constant for it (`OliveApi.childRibbon = '/v1/children/:childId/ribbon'`), with no `fetchRibbon()` method and no matching route in `server/routes.mjs`. Building this honestly — a real server-side timezone/schedule computation feeding a real `guardian_home_live.dart`, mirroring `child_home_live.dart`'s own shape — is a real, screen-sized feature, the exact kind of per-screen design judgment §19 reserves for deliberate scoping rather than folding into an unrelated pass. First confirmed genuinely real (not merely unwired) by the v0.49.15 dead-wire sweep's "Investigated, not changed" finding; re-confirmed unchanged when `main_live_guardian.dart` shipped in v0.49.30 and again at v0.49.39. Tracked here, not built. |
...
*End of MASTERFILE v0.49.42. Amend in place. Bump version. Log in CHANGELOG.
