...
**Separately found, not fixed this pass:** `packages/auth/src/auth.ts` also
exports `escalateSession()` — a server-side primitive that mints a real,
independently-TTL'd escalated session token — with no caller anywhere in
`server/` and no test file at all. Whether client-side escalation should
also mint one of these (giving an escalated guardian a live elevated API
session, not just a released kiosk lock) is an open question this pass
does not answer.
...
*End of MASTERFILE v0.49.7. Amend in place. Bump version. Log in CHANGELOG.
