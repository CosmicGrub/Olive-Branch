...
> **Both halves real as of v0.49.0.** `GET /v1/children/:id/export` (§7.9's
> own already-documented shape) is registered once (`server/routes.mjs`),
> dispatching on `?kind=`, and serves both:
>
> **Raw** — backed by `packages/db/src/pool.ts`'s `rawExportBundleFor()`,
> the default when `kind` is `raw` or omitted (matching the original,
> already-shipped client contract). Scoped to a live `guardian` edge
> (delivery_intent/media_artifact/message_log carry no RLS of their own —
> see that function's header for exactly what closes the gap), a real
...
> unmodified. A real denial (`tier_required` / `annual_allowance_used` /
> `chain_broken` / not-a-guardian) states its exact reason, never a
> fabricated success or a silent failure. `client/lib/court_export.dart`'s
> `LiveCourtExportScreen` calls it for real, falling back to the original
> synthetic demo chain only when no live backend is configured. See
> CHANGELOG.md's v0.49.0 entry for the full test/verification list.
