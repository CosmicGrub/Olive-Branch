...
### Verification
Real, computed assertion total: **5560** (3656 JS/server + 1904 client, client unchanged — this pass touches no Dart file). Confirmed via a full local `tools/verify.sh` run, genuinely 0 failed, not carried forward. `check-markup.mjs` passes all 44 checks. No live-device verification needed — every change in this pass is server/package-level and unit-tested.
