...
### Verified
`storage.test.mjs` 25/25 (7 new `verifySignedKey` assertions), `signed_media_route.test.mjs` 17/17 (new file), `inbox_route.test.mjs` extended (12/12, +2 assertions) — all against a real, freshly-migrated WSL Postgres; every other `tools/verify.sh` suite unchanged and still 0 failed (JS+SQL+demo computed total 4393). `guardian_more_test.dart`/`inbox_screen_test.dart`/`receipt_screen_test.dart` all clean, `flutter analyze` clean, native `flutter test` 1960/1960. This session's own split Windows/WSL environment cannot run `tools/verify.sh` as one process (Postgres lives in WSL, Flutter runs natively), so the 6353 total MARKUP.html/DEMO.html now claim is a manual reconstruction (4393 + 1960) rather than one script's own single printed number — flagged here rather than silently presented as equivalent, and worth re-confirming against a real single-machine CI run before this ships past review.
