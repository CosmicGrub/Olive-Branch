...
## [0.49.36] — 2026-08-24 — Real OS picture-in-picture for the child, accepted as an explicit tradeoff, plus the native containment fix it required and a new in-call activity to build on it

The "On the Line" audit (2026-08-23) scoped better PiP/multitasking support and, separately, the owner asked for the child's side of a call to gain PiP with a shared-activity feature paired to it. MASTERFILE §5.24.4 had called guardian-only PiP a *structural* conclusion — a child in kiosk lock cannot leave the app, so PiP "solves a problem she does not have," and a PiP window is not full-screen, so whatever sits behind it is reachable. That tension was disclosed to the owner directly, twice, across two rounds of clarification (the first-choice option — a shared canvas with no PiP — turned out to be technically infeasible against the current Jitsi SDK once actually investigated). The owner's second, informed choice: accept real PiP for the child, to make drawing together during a call genuinely simultaneous rather than sequential.

### Fixed — a critical kiosk re-pin bug, found and closed before any PiP-enabling code shipped
Tracing the real Android Activity lifecycle (not assumed) surfaced a genuine bug in the existing call-handoff design: `MainActivity.onResume()` fires for THREE different real events — a clean call end, a mid-call kiosk defeat, and a PiP entry (Android resumes the host Activity behind/around a floating PiP window). The original `consumeExpectingCallHandoff()` was a one-shot read-and-clear: whichever of those three resumed the Activity FIRST would consume the flag, leaving nothing to re-pin on a later, genuine call end — meaning a kiosk-locked child's device could end a real call and never re-lock, if PiP had been entered even once during that call. Fixed with a new signal, `KioskBridge.ACTION_CALL_ACTIVITY_DESTROYED`, broadcast from `WrapperJitsiMeetActivity.kt`'s own `onDestroy()` — the one real "this call Activity is gone for good" moment, distinct from every resume a PiP entry also produces. `KioskBridge.stillExpectingCallHandoff()` (peek, never consume) replaces the old consume-on-read method; `MainActivity.onResume()` now re-pins on every resume while a handoff is outstanding, and only the new destroyed-broadcast receiver ever actually clears the flag. Verified compiling clean (`./gradlew -q :app:compileDebugKotlin`, exit 0, only pre-existing benign warnings) before any Dart-side change was written.

### Added — real PiP for both roles, and a new in-call activity for the child
`call_screen.dart`'s `callFeatureFlagsFor()` now sets `pipEnabled`/`pipWhileScreenSharingEnabled` true unconditionally (previously `isGuardian`-only, the v0.49.35 fix's own default). No custom Flutter PiP button exists anywhere — deliberately, matching this file's own established reasoning: Jitsi's native in-call toolbar already offers PiP entry once the flag is set, and a Flutter-drawn button in this screen's own `build()` would never be reachable during a real call regardless, since Jitsi's own native Activity owns the entire display once joined.

A new top-level `shouldShowInCallActivities()` — pulled out as a pure function the same way `isGuardianWho`/`callFeatureFlagsFor` already are, specifically so it's directly unit-testable without needing a platform-channel-dependent join to reach `_status == inCall` in a test sandbox — decides when `_CallScreenState.didChangeAppLifecycleState` (new; the State now `with WidgetsBindingObserver`) should navigate: on `AppLifecycleState.resumed`, while genuinely in-call, not already shown once this call, and never for the guardian (her own device resuming behind her own PiP'd call should show whatever she was already doing, no redirect). When it fires, it opens a new, public `InCallActivitiesScreen` — the real, already-tested `DoodleDesk` engine, wrapped with honest, non-oversold copy ("This drawing isn't shared with his screen yet, but you can still show him what you made"). `InCallActivitiesScreen` is deliberately public, not `_`-prefixed like this file's other single-use display widgets, specifically so its content is verified directly in tests rather than only ever reachable on a live device.

**Deliberately incomplete, disclosed not hidden:** this does not build real-time canvas sync between the two devices. `annotation_canvas.dart`'s own header says plainly why a second actor doesn't exist there yet — "there is just one actor in this preview build because there's no realtime transport yet to carry a second" — directly correcting an earlier MARKUP/CHANGELOG claim the "On the Line" audit found (that live pairing already reused the shared annotation canvas "wired for two devices"; it was not). Building that transport is real, separately-scoped work, per the owner's own framing that joint/shared activities should unlock cleanly on top of the always-available solo library, not be faked into existing here.

### Verification
`flutter analyze` clean, zero issues, on `call_screen.dart` and the full client. 14 new/updated assertions in `call_screen_test.dart`: `callFeatureFlagsFor`'s PiP-flag test updated for the new both-roles-true reality; five new `shouldShowInCallActivities` cases (fires correctly; never for the guardian; never twice for the same call; never before a call is actually joined; never on a non-resumed lifecycle state, checked across every `AppLifecycleState` value); two new `InCallActivitiesScreen` tests (the honest disclosure copy renders; it genuinely wraps `DoodleDesk`, not a stub). Full client suite: **1911/1911 passing** (1897 + 14), run clean after confirming no stray leftover `dart.exe`/`dartvm.exe` process was contending for resources first.

**Real total: 5547, computed, not carried forward.** `tools/verify.sh`'s own real JS/server total is **3636** — confirmed by an actual full local run (WSL Postgres 16 + Node 22), combined with the native Windows Flutter 1911. Getting a genuine, complete number surfaced and fixed two real local-verification gaps unrelated to this PR's own diff, found rather than papered over with a guess: `packages/homework/test/make-fixtures.sh` was silently crashing on `set -euo pipefail` because this Windows checkout's working copy had picked up CRLF line endings despite `.gitattributes`' own `*.sh text eol=lf` rule (the committed blob is correctly LF — a stale working-tree artifact, not a repo defect, fixed locally without a commit) — this alone was masking 30 real assertions as a silent 0/0; separately, no local `livekit-server` binary existed for the `live (latency floor, pictionary)` suite's own `live.test.mjs`, masking another 21 — fetched the identical v1.8.0 Linux binary and config CI's own workflow uses, not a substitute. `MARKUP.html`/`shell.html`/`package.json`'s `check:markup` placeholder are all updated to the real 5547, not the prior 5540 carried forward unverified — the same class of mistake named and fixed once already (v0.49.34's own note) is not repeated here.

**Not yet independently verified live on a real device.** The native re-pin fix is reasoned correctly from reading `WrapperJitsiMeetActivity.kt`/`MainActivity.kt`/`KioskBridge.kt` directly, and compiles clean, but has not yet been confirmed against a real PiP entry and a real attempted escape on the Fold5 — specifically: that entering PiP genuinely resumes the Flutter host visibly; that `didChangeAppLifecycleState` actually fires and navigates correctly; that the native re-pin genuinely re-engages and holds against Recents/Home; and that a call TRULY ending (not merely PiP'd) still correctly re-pins. This is the explicit next step, tracked here rather than assumed complete.
