...
//
// This build points at the public meet.jit.si server so a real call can be
// proven end to end on two physical devices before a self-hosted Jitsi
// Videobridge stack exists. The one thing the two devices still need to
// agree on is which room to join, and that coordination reuses real, tested
// logic — packages/session-runtime/src/rooms.mjs's newRoomName()/mintToken(),
// which still enforce I1 (room name never guessable) and I4 (authorization
// gate) — served locally by scaffold/tools/local-call-room-server.mjs.
// That server is LOCAL DEV/TEST ONLY, not a stand-in for the production API.
//
