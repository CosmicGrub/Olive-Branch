...
void main() {
  ...
  group('responsive — comfortable reading width cap (form_factors.dart)', () {
    // Not child-facing (see file header). On a wide tablet/desktop viewport
    ...
    testWidgets('the cap engages only on a wide tablet/desktop viewport — '
      ...
      expect(t.getSize(find.byType(ListView).first).width, ff.comfortableReadingWidth);

      await pumpAt(const Size(344, 882)); // Fold5 cover
      expect(t.getSize(find.byType(ListView).first).width, 344);

      await pumpAt(const Size(390, 900)); // standard phone
      expect(t.getSize(find.byType(ListView).first).width, 390);
    });
  });
}
